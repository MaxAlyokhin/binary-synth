...
export function usePlaybackControl(file, settings, status, audioGraph, oscillatorScheduler, midiScheduler) {
    const nextListTimeoutID = ref(null)

    const listSize = computed(() => (settings.bitness === '8' ? 500 : 250))
    const isOnlyOneList = computed(() => listSize.value >= settings.fragment.to - settings.fragment.from)
    ...
    function endOfListFor(startOfList) {
        return startOfList + listSize.value <= settings.fragment.to ? startOfList + listSize.value - 1 : settings.fragment.to
    }
    ...
    function nextList(listID, startOfList) {
        ...
        if (startOfList >= settings.fragment.to) {
            // In midi mode, we turn off the last note
        ...
        // We are planning the next list
        // If there is only one list
        if (isOnlyOneList.value) {
            if (!settings.loop) {
                ...
                // In the looped mode on a single list
                nextListTimeoutID.value = setTimeout(() => {
    ...
    function setupPlaybackWatchers(bynaryInSelectedBitness) {
        ...
        // If bitness changed, recalc current list bounds
        watch(
            () => settings.bitness,
            () => {
                const endOfList =
        ...
        watch(
            [() => settings.fragment.from, () => settings.fragment.to],
            () => {
                const endOfList =
                    settings.fragment.from + listSize.value < settings.fragment.to
                        ? settings.fragment.from + listSize.value - 1
